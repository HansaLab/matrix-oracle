...
function setBackgroundVisual(url, durationMs) {
    ...
    if (url === 'hmm.gif') {
        ...
        setTimeout(() => { leftCat.remove(); rightCat.remove(); }, durationMs);
    } else {
        const gifImg = document.createElement('img');
        gifImg.src = url;
        gifImg.className = 'special-bg-gif active-special';
        document.body.appendChild(gifImg);
        setTimeout(() => { gifImg.remove(); }, durationMs);
    }
}
...
// FUNKCE PRO KONEC - S TANCOVÁNÍM
function triggerShutdown() {
    isDead = true;
    const overlay = document.getElementById('idiot-overlay');
    if(overlay) {
        overlay.style.backgroundImage = "url('dance.gif')";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(input) {
        input.oninput = (e) => monitorInput(e.target.value);
